...
def generate_launch_description():
    ...
    ld = launch.LaunchDescription([
        launch.actions.DeclareLaunchArgument(name='robot_id', default_value='robot1_104'),
        ...
        #  Ajout du WebSocket Bridge
        launch.actions.IncludeLaunchDescription(
            launch.launch_description_sources.PythonLaunchDescriptionSource(
                os.path.join(get_package_share_directory('websocket_bridge'), 'launch', 'websocket_bridge.launch.py')
            )
        ),

        #  Ajout du Service Identifier
        launch.actions.IncludeLaunchDescription(
            launch.launch_description_sources.PythonLaunchDescriptionSource(
                os.path.join(get_package_share_directory('identifier_command'), 'launch', 'identifier_command.launch.py')
            ),
            launch_arguments={
                'topic_prefix': launch.substitutions.LaunchConfiguration('robot_id'),
        ...
        #  Ajout du package Mission
        # launch.actions.IncludeLaunchDescription(
        #     launch.launch_description_sources.PythonLaunchDescriptionSource(
        #         os.path.join(get_package_share_directory('mission'), 'launch', 'mission.launch.py')
        #     )
        # ),

        #  Ajout du package Random Walk
        launch.actions.IncludeLaunchDescription(
            launch.launch_description_sources.PythonLaunchDescriptionSource(
                os.path.join(get_package_share_directory('random_walk'), 'launch', 'random_walk.launch.py')
            )
        ),
