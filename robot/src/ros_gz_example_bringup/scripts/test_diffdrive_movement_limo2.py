...
class Limo2Test(Node):
    def __init__(self):
        super().__init__('limo2_test')
        self.publisher = self.create_publisher(Twist, 'limo2/cmd_vel', 10)
        
        self.get_logger().info("==================================================")
        self.get_logger().info("LIMO2 TEST STARTING")
        self.get_logger().info("==================================================")
    ...
    def timer_callback(self):
        if self.phase == 0:
            if self.count < 30:  # 3 seconds forward
                ...
                self.msg.angular.z = 0.8  # left turn
            ...
            if self.count < 30:  # 3 seconds turn
                ...
                self.msg.linear.x = 0.5
                self.msg.angular.z = -0.8  # right turn
            ...
            if self.count < 10:
                ...
                self.phase = 5
                self.get_logger().info("==================================================")
                self.get_logger().info("LIMO2 TEST COMPLETE")
                self.get_logger().info("==================================================")
                self.timer.cancel()
                self.destroy_node()
                rclpy.shutdown()
