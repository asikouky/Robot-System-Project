...
def generate_launch_description():
    ...
    generate_simulation_world = ExecuteProcess(
        cmd=[
            ...
            '--sdf-source',
            os.path.join(pkg_project_gazebo, 'worlds', 'diff_drive_world_source.sdf'),
            '--sdf-dest',
            os.path.join(pkg_project_gazebo, 'worlds', 'diff_drive_generated.sdf'),
        ],
        output='screen'
    )
    ...
    package='websocket_bridge',  # remplace par le bon package si différent
        executable='websocket_ros_bridge',  # le nom défini dans setup.py
        name='websocket_bridge',
        output='screen',
    )

    mission = Node(
    package='mission',  # remplace par le bon package si différent
        executable='mission',  # le nom défini dans setup.py
        name='mission',
        output='screen',
    )
    ...
    random_nav_limo1 = Node(
        ...
        namespace='limo1',
        output='screen',
        parameters=[{'enable_ackerman': False}]
    )
